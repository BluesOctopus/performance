...
| 分词器          | MDL 最优 K | K处压缩率 | L_total 最优 budget | 最优 MDL 降低 |
| ------------ | -------- | ----- | ----------------- | --------- |
...
| **词法前缀/后缀**  | `self.`, `np.`, `get`_, `test_` 等 400 种 |
...
1. **贪心 K 偏保守**：K ≈ 60 远低于经验最优 ≈ 500-1000。贪心逐步决策可能低估批量添加的协同效应。
2. **Uniform code 假设**：L_data 使用 `N × log₂(V)` 假设所有 token 等概率，实际上 token 分布高度不均匀。用实际熵替代 log₂(V) 可能给出更精确的 MDL 估计。
