...
def build_local_replacement_map(
    ...
    import keyword
    # Markers to protect: SYN_n, <SYN_n>, <D>, <I>, <VAR>, <ATTR>, <STR>, <FSTR>, <NUM>, <Vn>, <R>, </R>
    ...
    for word, freq in candidates.items():
        # Use simple token count for the word with global cache
        if word not in _ENCODE_CACHE:
            _ENCODE_CACHE[word] = len(mc_encode(tokenizer, tok_type, word))
        orig_toks = _ENCODE_CACHE[word]
        
        # Overhead: word tokens + 1 for separator (comma)
        overhead = orig_toks + 1
        # Savings: (orig - 1) * freq - overhead
        # (orig - 1) because the placeholder <Vi> is 1 token
    ...
    # Compact header: <R>word1,word2,...</R>
    # Placeholders: <V0>, <V1>, ...
    rmap = {}
    registry_words = []
...
def apply_token_replacement(text: str, rmap: dict[str, str]) -> str:
    ...
    # Pre-categorize the replacement map based on the stable markers
    # We look for markers like <VAR_N>, <STR_N>, etc.
