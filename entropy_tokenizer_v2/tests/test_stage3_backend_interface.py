...
def test_stage3_backend_interface_hybrid_ab_smoke_missing_cfg() -> None:
    ...
    assert res.vocab_entries == []
    assert "stage3_ab_runtime_warning" in res.metrics


def test_backend_registry_resolves_known_names() -> None:
    assert get_stage3_backend("legacy").name == "legacy"
    assert get_stage3_backend("plan_a").name == "plan_a"
    assert get_stage3_backend("hybrid_ab").name == "hybrid_ab"
