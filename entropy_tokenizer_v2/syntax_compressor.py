...
def mine_block_patterns(sources: List[str], min_freq: int = AST_MIN_FREQ) -> Counter:
    ...
    print(f"[mine] Mined {len(counter)} unique patterns (before freq filter)")
    filtered = Counter({k: v for k, v in counter.items() if v >= min_freq})
    print(f"[mine] {len(filtered)} patterns remain after freq >= {min_freq}")
    return filtered
...
def decompress_source_syntax(compressed_text: str, selected: List[SkeletonCandidate]) -> str:
    ...
    sk_info = []
    for c in selected:
        sk = c.skeleton # Single line only now
    ...
    for line in lines:
        ...
        if num_slots > 0:
            ...
            try:
                # Restore spaces in slots
                slots = [s.replace("__SP__", " ") for s in slots]
                restored = sk.format(*slots)
                # Append restored header and any remaining text on the line
